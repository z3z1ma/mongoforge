...
export function createMutateCommand(): Command {
  ...
  command
    .description('Run update and delete workloads against existing MongoDB data')
    ...
    .option('--ratio <ratio>', 'Operation ratios (e.g., update:70,delete:30)', 'update:100')
    ...
    .option('--update-strategy <strategy>', 'Update strategy: regenerate, partial, mixed', 'partial')
    .action(async (options) => {
      try {
        const schema = await loadGenerationSchema(options.schema);
        const dbName = options.database || new MongoClient(options.uri).db().databaseName || 'test';
        ...
        const cursor = client.db(config.database).collection(config.collection)
